function RESULTS = GEA_tabu(model, numberOfRuns, maxIt, nPop)
    RESULTS = [];
    runsCount = 0;
    ...
    for it = 1:maxIt
        ...
        F1=pop(F{1});
        result.GEA_tabu=[result.GEA_tabu; F1(1).Cost(1)];
        disp(['(GEA_tabu, run: ' num2str(runsCount) ') Iteration ' num2str(it) ': Number of F1 Members = ' num2str(numel(F1)) '; Best cost ' num2str(result.GEA_tabu(it))]);
    end
